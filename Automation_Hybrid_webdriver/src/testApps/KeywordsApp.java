...
public class KeywordsApp extends DriverApp {
	...
	public static String getText() throws Exception{
		...
		APPLICATION_LOGS.debug("Validating input Keyword");
		// extract the test data
		String message = "pass";
		String data =testData.getCellData(currentTest, data_column_name , testRepeat);
		...
		try
		{
				...
				Boolean iselementpresent = driver.findElements(By.xpath(Objects.getProperty(object))).size()!= 0;
				   if (iselementpresent == true)
				   {			   
					   if(driver.findElement(By.xpath(Objects.getProperty(object))).getText().contains("The email and password you entered don't match"))
				       {
				      	return "Fail";
				      }
		...
		{
				// report error
			APPLICATION_LOGS.debug("Error while validating input -" + object + t.getMessage());
